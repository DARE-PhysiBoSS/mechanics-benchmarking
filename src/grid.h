...
public:
	...
	void insert_agent(const real_t* position, std::size_t agent_id);
	...
	const std::vector<std::size_t>& get_moore_indices(std::size_t voxel_index) const;

	bool is_grid_2d() const;

	std::size_t voxel_index(const real_t* position);

	std::vector<std::size_t> get_grid_coordinates(const real_t* position);
